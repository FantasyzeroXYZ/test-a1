...
import { prefixInflection, suffixInflection } from './deinflector';
import type { LanguageTransforms, Condition, TransformRule } from './deinflector';

/** @typedef {keyof typeof conditions} ConditionKey */

/**
 * @param {string} consonants
 * @param {string} suffix
 * @param {ConditionKey[]} conditionsIn
 * @param {ConditionKey[]} conditionsOut
 * @returns {import('./deinflector').TransformRule[]}
 */
function doubledConsonantInflection(consonants: string, suffix: string, conditionsIn: string[], conditionsOut: string[]): TransformRule[] {
    const inflections: TransformRule[] = [];
...
const pastSuffixInflections: TransformRule[] = [
...
const ingSuffixInflections: TransformRule[] = [
...
const thirdPersonSgPresentSuffixInflections: TransformRule[] = [
 ...
 */
const phrasalVerbInterposedObjectRule: TransformRule = {
    type: 'other',
 ...
 */
function createPhrasalVerbInflection(inflected: string, deinflected: string): TransformRule {
    return {
        type: 'other',
 ...
 * @returns {import('language-transformer').Rule<Condition>[]}
 */
function createPhrasalVerbInflectionsFromSuffixInflections(sourceRules: TransformRule[]): TransformRule[] {
    return sourceRules.flatMap(({isInflected, deinflected}) => {
        if (typeof deinflected === 'undefined' || !isInflected) { return []; }
...
export const englishDeinflectionRules: LanguageTransforms = {
    language: 'en',
    conditions,
    transforms: {
        'plural': {
